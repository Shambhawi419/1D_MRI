"""
Evaluation script for the weight-sharing-only ablation, mirroring
../odls/evaluate.py.
"""
...
def reconstruct_slice(model: ODLSAblationSharing, k_space_slice: np.ndarray, mask: np.ndarray,
                       device: str) -> np.ndarray:
    M, N, J = k_space_slice.shape
    hybrid_full = fe_ifft(k_space_slice)

    z_rows = hybrid_full * mask[None, :, None]
    z_ch = np.stack([_to_real_imag_channels(z_rows[m]) for m in range(M)], axis=0)
...
def evaluate_dataset(model: ODLSAblationSharing, volumes: List[np.ndarray], mask_type: str,
...
def build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Evaluate a weight-sharing-only ablation checkpoint")
...
def main():
    ...
    model = ODLSAblationSharing(n_coils=args.n_coils, n_phases=args.n_phases, width=args.width,
